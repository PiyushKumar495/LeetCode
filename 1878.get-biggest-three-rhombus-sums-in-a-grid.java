class Solution {
    ...
    public int calculateSum(int row, int col, int[][]grid, int sideLength){
        int sum = 0;
        for(int side=0;side<sideLength;side++) sum+=grid[row++][col++];
        for(int side=0;side<sideLength;side++) sum+=grid[row++][col--];
        for(int side=0;side<sideLength;side++) sum+=grid[row--][col--];
        for(int side=0;side<sideLength;side++) sum+=grid[row--][col++];
        return sum;
    ...
    public int[] sortedResult(Set<Integer> set){
        // handles the edge case of [7,7,7]
